    ...
    <div class="teacher-form-card teacher-mobile-card">
        ...
        <div class="d-flex flex-column gap-3 mb-4">
            <div class="p-3 rounded border bg-light">
                <div class="fw-semibold mb-1">{{ $complaint->teacher?->name ?? 'الأستاذ' }}</div>
            ...
            @foreach($complaint->messagesChronological as $message)
                <div class="p-3 rounded border {{ $message->sender_role === 'teacher' ? 'bg-primary text-white' : 'bg-light' }}">
        ...
        <form action="{{ route('teacher.complaints.reply', $complaint->id) }}" method="POST">
            @csrf
            <textarea name="message" class="form-control teacher-form-control mb-3" rows="4" required @disabled(! $canReply)>{{ old('message') }}</textarea>
            <button type="submit" class="btn teacher-btn-primary px-4" @disabled(! $canReply)>إرسال الرد</button>
        </form>
    </div>
@endsection
